...
8. 画像入力パイプラインを実装する。grant 付き Uri を受信したら即座に内部キャッシュ `filesDir/cache/{uuid}.jpg` へコピーし、その後に downscale と JPEG 化を行う ImageDownscaler を用意する。`ContentResolver` で開いて `Dispatchers.IO` 上でデコード・縮小・JPEG エンコードし、リサイズ ON のとき長辺 1568px・JPEG quality 85、リサイズ OFF のときも (a) Uri の内部キャッシュコピー、(b) JPEG への正規化、(c) フェイルセーフとして長辺 4096px / 出力 4MB を上限とする再縮小、までは必ず実施する（Gemini への過大送信防止）。履歴へ保存する場合のみ `filesDir/history/{id}.jpg` へ昇格し、未保存の temp ファイルはアプリ起動時の sweep でクリーンアップする。共有元の grant Uri は `takePersistableUriPermission` が効かないケースがあるため、永続化は試みず内部コピー一択で運用する。_parallel with 5,6,7 after 4_
...
12. 共有受信エントリを実装する。ShareReceiverActivity を透明 Activity として追加し、`ACTION_SEND text/plain`、`ACTION_SEND image/*`（受け入れ MIME は `image/png` / `image/jpeg` / `image/webp` を一次対象）、`ACTION_SEND_MULTIPLE image/*`、`ACTION_PROCESS_TEXT`（intent-filter に `android:processable="true"` を付与、結果テキストは送り返さない読み取り専用扱い）を受けて、入力正規化後に ResultActivity を起動する。ResultActivity 起動 Intent には Uri 文字列のみを extras で載せ、`Intent.FLAG_GRANT_READ_URI_PERMISSION` を再付与する（Bitmap や生バイトは絶対に載せない）。text は 50,000 文字（または 200KB）を閾値にそれ以下は extras、超過時はアプリ内 cache file へ書き出して path のみ受け渡す。API キー未設定時は **MainActivity を経由させ、起動時の onboarding バナーから設定画面へ誘導する**（共有経由でいきなり設定画面に飛ばす経路は採らない）。_depends on 5,10,11_
13. 設定画面を実装する。API キー入力、表示トグル、接続テスト、モデル選択、ベースシステムプロンプト、画像リサイズ ON/OFF、ストリーミング ON/OFF を Compose で実装し、保存先は 5 の preferences に統一する。接続テストは 7 の GeminiClient を呼び、成功・認証失敗・通信失敗を UI に明確に返す。_depends on 5,7; parallel with 10-12 after 5,7_
...
19. 統合受け入れを行う。Chrome 共有テキスト、ギャラリー共有画像、テキスト選択 ACTION*PROCESS_TEXT、API キー未設定、認証失敗、履歴再実行、複数画像共有、回転中ストリーミング継続、CI 署名付き Release 生成を実機またはエミュレータで検証し、Phase 1 完了条件を満たすことを確認する。\_depends on 11-18*
...
- 画像キャッシュ命名は `filesDir/cache/{uuid}.jpg`、履歴は `filesDir/history/{id}.jpg` で固定。
...
- `ACTION_PROCESS_TEXT` は `processable="true"` で読み取り専用として扱い、結果テキストの送り返しはしない。
...
- 画像リサイズ OFF でも Uri コピー・JPEG 正規化・長辺 4096px / 4MB のフェイルセーフ縮小は必ず実施する。
...
- 設定とセキュリティ: API キーが EncryptedSharedPreferences に暗号化保存され、未設定判定が共有起動経路でも機能する（Verification 2）。
...
- 画像処理: grant Uri 失効後も内部コピーで処理が継続し、リサイズ ON/OFF の双方でフェイルセーフ上限を超えない（Verification 5）。
