...
- `SettingsActivity` (Compose) を実装
  ...
  - ストリーミングON/OFF（既定ON）
  - 履歴保持件数 / 保持日数（既定 200 件 / 90 日、pinned は pruning 対象外）
...
- `ImageDownscaler`: リサイズ ON で長辺 1568px / JPEG quality 85 に縮小。リサイズ OFF の場合、元画像が WebP/JPEG/PNG かつフェイルセーフ（長辺 4096px / 4MB）以下なら再エンコードせず内部コピーをそのまま送信し、条件を超える場合または非対応形式の場合のみ再縮小や JPEG 化を行う（Gemini への過大送信防止）
  - 入力 `Uri` → `ContentResolver` で開き、`Dispatchers.IO` 上でデコード・縮小・必要に応じた再エンコードを行う
  - 出力先は `filesDir/cache/{uuid}.{ext}`（再エンコード時は `.jpg`、パススルー時は元形式に応じた拡張子を保持）。履歴採用時は `filesDir/history/{id}.{ext}` へ昇格
...
- `ShareReceiverActivity`（透明 Activity）を AndroidManifest に登録:
  ...
  - `ACTION_PROCESS_TEXT`（テキスト選択メニュー「翻訳(Gem Read)」、intent-filter には標準の action と `text/plain` data のみを設定し、Activity 側で `setResult()` を呼ばないことで元のテキストを置換しない読み取り専用として振る舞う）
...
- 画像は内部ストレージ（`filesDir/history/{id}.{ext}`）に保存し、再エンコード時は `.jpg`、パススルー時は元形式に応じた拡張子を保持する
...
12. 画像リサイズ OFF でも Uri コピーが行われ、フェイルセーフ以下の WebP/JPEG/PNG はパススルーされ、条件超過時または非対応形式では 4096px / 4MB 上限に収まるよう再縮小・必要に応じて JPEG 化されること
...
- **画像リサイズ OFF 時のフェイルセーフ**: 元画像が WebP/JPEG/PNG かつ長辺 4096px / 4MB 以下なら再エンコードせずパススルーし、条件超過時または非対応形式では再縮小・必要に応じた JPEG 化を行う
- **ACTION_PROCESS_TEXT**: マニフェストの特別な属性を使わず、Activity 側で `setResult()` を返さない実装とすることで、元のアプリのテキストを置換しない読み取り専用として扱う
